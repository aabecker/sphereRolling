function QuatGradDec(ang_X,ang_Y,nr)
...
%For this one, I try to implement the method using quaternions
format compact
clc
tic
...
%     temp1=temp*[0;0;1];
%     obj_f{n}=atan2(real(temp1(2)),real(temp(1)));
% end
...
%         temp=temp1*[0;0;1];
%         obj_temp=atan2(real(temp(2)),real(temp(1)));
%         ffc=ffc+obj_temp; %get the objective function value for later comparison
...
%alignment angle.
%Michael Williams 2015, e-mail:michael.williams.hy@gamil.com
StepLength=0.1*pi; %length for trial
align_error=1;  %precision
trial=0;  %initialize
numBall=numel(rads);
Xtemp=repmat(eye(3),[1,1,numBall]);
theta2=zeros(1,numBall);
while align_error>0.05
    trial=trial+StepLength;
    for n=1:numBall
        Xtemp(:,:,n)=RotateZ(trial/rads(n))*X(:,:,n);
        temp=Xtemp(:,:,n)*[0;0;1];
        theta2(n)=atan2(real(temp(2)),real(temp(1)));
    end
    align_error=sum(abs(theta2));
end
Zturn=trial;
end
