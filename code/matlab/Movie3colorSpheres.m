function Movie3colorSpheres
% repeatable random number generation:
rng('default'); rng(1);
% use functions
% Possibilities:  color the sphere with 3 colors (r,g,b), design control
...
%one way to make movies.  Aaron has a better way now.
...
for k = 1:numStep
    if sum( k ==  goalTimes)  % for making movie -- saves static frames at beginning and end
       ...
       % update the goal after specified number of steps
    if(     k < goalTimes(2))
        Goal = GoalU;
